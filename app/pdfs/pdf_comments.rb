...
class PdfComments < Prawn::Document
  ...
  def generate
    if @session_activities and @session_activities.size > 0
      ...
      if @activityType == 'Curs'
        questions = @current_session.form.get_text_questions
      else
        if @activityType == 'Seminar'
          questions = @current_session.formS.get_text_questions
        else
          if @activityType == 'Laborator'
            questions = @current_session.formL.get_text_questions
          else
            questions = @current_session.formP.get_text_questions
          end
        end
      end
      ...
      questions.each do |question|
        font('public/dejavu-sans/DejaVuSans.ttf') do
          text question[:text]
        end
